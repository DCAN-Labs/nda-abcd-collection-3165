...
## DAIRC Quality Control (QC) Selection Process
...
## Field Map Selection Process

A single pair of positive (posterior to anterior) and negative (anterior to posterior) spin echo field maps are acquired along with each functional scan. Fieldmaps are used during image preprocessing to correct for distortions in the phase encoding direction of each fMRI volume using [FSL's topup](https://fsl.fmrib.ox.ac.uk/fsl/fslwiki/topup). 

These field maps are highly susceptible to motion artifacts and can have a negative effect on processing if they are not high-quality. Because field maps are largely consistent between runs, we select only a single pair of spin echo field maps to use for all anatomical and functional scans for distortion correction. Selection is based on the field map pair with the least variance from the registered group average (this assumes that field maps with large motion artifacts will have more noise and higher variance from the average). This fmap pair is to applied to each functional scan to avoid processing with a poor-quality field map.

The chosen pair of field maps list all anatomical and functional runs in the `IntendedFor` field of their sidecar JSONs.
...
The bval and bvec files associated with the DWI data for each scanner and software version were provided by the DAIRC and can be found in the [nda-abcd-s3-downloader](https://github.com/ABCD-STUDY/nda-abcd-s3-downloader) repository. This was due to formatting issues and irregularities with these files that were packaged along with the DICOMs.
